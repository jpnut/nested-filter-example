...
import { FieldProps, nullOperator, operatorToString as defaultOperatorToString, Operators } from 'react-nested-filter';
...
export const DateField: React.SFC<FieldProps> = ({ value, setValue, operator, setOperator, operators }) => {
  ...
  const selectedDate = React.useMemo(() => (isNil(value) ? null : dateFromString(value)), [value]);
  ...
  return (
    <>
      ...
      <Box display="flex" flex={1} style={styles}>
        <DatePicker clearable={true} value={selectedDate} onChange={handleChange} fullWidth />
      </Box>
    </>
  );
};
